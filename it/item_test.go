...
func (suite *TestingSuite) TestDoneItem() {
	listInput := fmt.Sprintf(
		`{"title": "%s", "description": "%s"}`,
		listForCreate.Title, listForCreate.Description,
	)
	...
	signInInput := fmt.Sprintf(
		`{"email": "%s", "password": "%s"}`,
		doneItemUser.Email, defaultPassword,
	)
	...
	for _, tc := range tests {
		suite.T().Run(tc.name, func(t *testing.T) {
			code, responseData := helpers.MakeRequest(
				...
				t,
				http.MethodPatch,
				fmt.Sprintf("/api/lists/%d/items/%d/done", tc.listID, tc.itemID),
				bytes.NewBuffer([]byte{}),
				headers,
			)
			...
			if tc.expErrMsg != "" {
				...
				t.Run("Check done result", func(t *testing.T) {
					code, getData := helpers.MakeRequest(
						suite.router,
						t,
						http.MethodGet,
